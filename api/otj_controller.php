<?php 
	...
	if ($data->method == 'save_otj_service') {
		...
		$sql = "INSERT INTO otj_customer_details (customer_id, enquiry_type, follow_up_type, assigned_employee_id, service_name, action_date,start_date,end_date, aum_price, short_desc) VALUES (:customer_id, :enquiry_type, :follow_up_type, :assigned_employee_id, :service_name, :action_date, :start_date, :end_date, :aum_price, :short_desc)";

		$sth = $dbh->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
		$result = $sth->execute(array(':customer_id' =>$data->customer_id,':enquiry_type'=>$data->enquiry_type,':follow_up_type'=>$data->follow_up_type, ':assigned_employee_id'=>$data->assigned_employee_id, ':service_name'=>$data->service_name,':action_date'=>$data->enquiry_date,':start_date'=>$data->start_date,':end_date'=>$data->end_date, ':aum_price'=>$data->price, ':short_desc'=>$data->short_description));
		//print_r($sth->errorInfo());
		if ($result == 1) {
			$data = array(
				"status" => true,
