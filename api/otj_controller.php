	...
	if ($data->method == 'save_otj_service') {
	    ...
	    if ($result == 1) {
			if ($data->assigned_employee_id != '0') {
				...
				$result = $sth->execute(array(':employee_id' => $data->assigned_employee_id));
				if ($result == 1) {
					$sql_employee = "INSERT INTO employee_assigned_service_details(employee_id,assigned_date, assigned_service) VALUES (:employee_id, :assigned_date, :assigned_service )";
					$sth = $dbh->prepare($sql_employee, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
					$result = $sth->execute(array(':employee_id' =>$data->assigned_employee_id,':assigned_date'=>$data->start_date,':assigned_service'=>$data->service_name));
				} else {
					}			
			}
	        $data = array(
